...
## 12. Agent Monitor — stray/"rotating" line bug

File: `apps/web/components/agents/agent-flow.tsx`

Reported symptom: on the Agent Monitor page, a line near the "AI Multi-Agent Orchestrator" node appears to be rotating/spinning oddly rather than sitting still as a normal connector.

What's confirmed from the code:
- All connector lines are SVG `<path>` elements generated by `buildEdges()` (~line 66) from the `AGENTS` array's `deps` relationships, rendered inside `<svg className="af-edges">` (~line 357).
- The `orchestrator` node has `deps: []`, so no edge should ever terminate *into* it — it only has outgoing edges (to `website`, `linkedin`, `reddit`).
- The visual "flow" animation on an active edge comes from CSS: `.af-edge.active { stroke-dasharray:7 9; animation:af-dash 1s linear infinite; }` (~line 585) plus animated `<circle>` "particles" traveling along the path via `<animateMotion>` (~line 372-377). This is intentional motion for *active* edges — but on a short or steeply-angled segment, the moving dash pattern combined with the particles can visually read as "spinning" rather than "flowing," which may be what's being perceived as a bug even if it's not a positioning error.
- There's also a separate, deliberately spinning element: `.af-ring` (~line 596) — a `conic-gradient` ring with `animation:af-spin-ring 1.3s linear infinite` that wraps *around* whichever node has `status === "running"` as a loading indicator. If this ring is rendering with incorrect sizing/position (e.g. around the wrong node, or extending past its container), it could look like a rotating line rather than a rotating ring.

Not yet confirmed (needs visual repro in a running instance, not just static code reading): whether the issue is (a) the `.af-ring` spinner rendering misplaced/malformed, (b) an edge whose `d` path coordinates are wrong for a specific node pair, or (c) a scroll/overflow clipping issue in `.af-scroll`/`.af-canvas` (~line 355-356) cutting off part of a longer curve so only a fragment is visible, making its dash animation look like independent rotation.

Fix approach: reproduce it live, identify which of the three above it actually is, then fix at the source (edge path math in `buildEdges()`, or the `.af-ring` sizing/positioning CSS, or canvas overflow) rather than just hiding the animation.

## 13. Four-Stage UI Enhancement Plan (overall visual overhaul)

Goal: make the whole app feel more professional, distinctive, and polished — new theme direction, refined typography, real hover/interaction states, and tasteful motion — without touching business logic, data, or routes (except the two authorized fixes already covered in Sections 11 and 12).

Use the installed "UI UX pro max" skill during every stage where applicable for design-quality guidance (typography pairing, color theory, spacing rhythm, motion timing).

Each stage below is deliberately scoped narrow so a single Claude Code prompt referencing "do Stage N" stays small. Do the stages **in order** — each one builds on the last.

### Stage 1 — Foundation: design tokens, fonts, and the two open bugs
Scope: `apps/web/app/globals.css`, `apps/web/tailwind.config.ts`, plus the two isolated bug fixes.
- Fix the Agent Monitor rotating-line issue (Section 12).
- Decide and implement a refreshed color token system in `globals.css` — resolve the existing gap between the current navy/blue theme and the unused `design.md` warm-token direction (Section 5/6): either adopt `design.md`'s tokens fully, evolve them into something even more distinctive, or propose a new direction — but land on ONE coherent palette and update the CSS custom properties in `:root` accordingly.
- Refine the type system: confirm/upgrade the DM Sans + Plus Jakarta Sans pairing (or propose a better pairing), set a clear type scale (display/heading/body/label sizes and weights) and apply it to the root tokens.
- Do NOT yet touch individual component files beyond `globals.css`/tailwind config — that's Stage 2. This stage is intentionally CSS-token-only plus the one bug fix, to keep it fast and cheap.

### Stage 2 — Apply the new system to core layout & shared chrome
Scope: `apps/web/components/layout/sidebar.tsx`, `apps/web/components/layout/app-shell.tsx`, `apps/web/components/dashboard/dashboard.tsx`, `apps/web/components/common/feature-page.tsx`.
- Restyle these highest-traffic files using the Stage 1 tokens: sidebar, topbar, metric cards, dashboard charts/tables, and the shared `FeaturePage` template (which drives 5 routes: accounts... wait, Accounts is removed — driven by analytics, documents, notifications, settings routes plus any others still using it).
- Refine card elevation/shadow, border radius consistency, and spacing rhythm across these files.
- Keep all existing data/content — this is a visual pass only.

### Stage 3 — Interaction & motion layer
Scope: sitewide hover/focus states, transitions, and the Outreach Studio enhancement.
- Add/refine hover and focus states across nav links, buttons, cards, and tabs sitewide (many currently have minimal or no hover feedback).
- Add tasteful micro-animations: button press feedback, card hover lift, tab-switch transitions, loading-state polish — keep transitions under ~250ms per the existing motion guidance in `design.md`, and avoid heavy motion in dense tables/analytics views.
- Finish the Outreach Studio enhancement from Section 11.4 that's still outstanding (e.g. company avatar chips are done per the latest screenshot — verify what remains: per-field empty/loading states, email/LinkedIn preview visual treatment, micro-interactions on tab switch and copy-button success state).

### Stage 4 — Bespoke components & final consistency pass
Scope: `research-center.tsx`, `intent-center.tsx`, `stakeholder-center.tsx`, `strategy-center.tsx`, `agent-flow.tsx`, `campaign-modal.tsx`, plus all auth pages and the onboarding flow.
- Bring these remaining custom-styled components (several of which use Tailwind utility classes directly rather than the shared CSS system) in line with the finished design system from Stages 1-3.
- Do a full consistency audit: typography scale, color usage, spacing, radii, and hover/motion patterns should feel identical across every screen by the end of this stage.
- This is the last stage — after this, the whole app should read as one coherent, professional product rather than a set of independently-styled pages.
